...
void benchmarkTransfer(size_t sizeGB) {
    ...
    // Warm-up transfer
    CUDA_CHECK(cudaMemcpyAsync(d_data, h_data, sizeBytes, cudaMemcpyHostToDevice, 0));
    ...
    start = getTime();
    CUDA_CHECK(cudaMemcpyAsync(h_data, d_data, sizeBytes, cudaMemcpyDeviceToHost, 0));
    CUDA_CHECK(cudaDeviceSynchronize());
    end = getTime();
